...
void Player::KeyCheck(const _float& dt)
{
	...
	static _bool isZeroDown = false;
	if (m_pInputDevice->IsKeyDown(GLFW_KEY_0))
	{
		if (!isZeroDown)
		{
			isZeroDown = true;
			m_pMesh->SetSelcted(!m_pMesh->GetSelected());
		}
	}
	else
		isZeroDown = false;
	...
	, vec3 vRot, vec3 vScale)
{
	...
	if (nullptr != m_pMesh)
	{
		...
		//*pmat = translate(*pmat, vec3(0.f, 8.7f, 0.f));
		//*pmat = rotate(*pmat, radians(-90.f), vec3(1.f, 0.f, 0.f));
