...
void SceneDungeon::LoadTileObjects(vector<Tile*>& tiles)
{
	...
	if (nullptr != m_pDefaultCamera)
	{
		...
		m_pDefaultCamera->SetCameraEye(vec3(vMiddlePos.x, vMiddlePos.y + 100.f, vMiddlePos.z - 100.f));
		m_pDefaultCamera->SetPlayer(m_pPlayer);
		m_pDefaultCamera->ChangeFollowing();
	}
