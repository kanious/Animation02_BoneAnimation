...
DefaultCamera::DefaultCamera()
	...
	, m_fSpeed(40.f), m_fRotSpeed(20.f), m_pTarget(nullptr), m_fDistance(15.f)
	, m_eCurDir(CD_DOWN), m_ePrevDir(CD_DOWN), m_fHeight(20.f), m_fDist(20.f)
	, m_iZoomLevel(5), m_iZoomMinLevel(1), m_iZoomMaxLevel(10), m_bFollowing(false)
{
...
void DefaultCamera::ChangeFollowing()
{
	m_bFollowing = !m_bFollowing;
	if (m_bFollowing)
	{
		m_vPrevEye = m_pCamera->GetCameraEye();
		m_vPrevTarget = m_pCamera->GetCameraTarget();
	}
	else
	{
		m_pCamera->SetCameraEye(m_vPrevEye);
		m_pCamera->SetCameraTarget(m_vPrevTarget);
	}
}
